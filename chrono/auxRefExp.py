...
elif local and not frameShift:  
    rev_2 = chrono.ChLinkRevolute()                                                     
    ground_frame = chrono.ChFrameD(chrono.ChVectorD(0,0,-1))            # local frame of attachment
    pend_2_frame = chrono.ChFrameD(chrono.ChVectorD(-1,0,0))            # local frame of attachment
    rev_2.Initialize(ground,pend_2,local,ground_frame,pend_2_frame)    # init joint
elif local and frameShift:
    rev_2 = chrono.ChLinkRevolute()                                                     
    ground_frame = chrono.ChFrameD(chrono.ChVectorD(0,0,-1))            # local frame of attachment
    pend_2_frame = chrono.ChFrameD(chrono.ChVectorD(0,0,0))             # local frame of attachment
    COG_to_REF = pend_2.GetFrame_COG_to_REF()
    rev_2.Initialize(ground,pend_2,local,ground_frame, ((pend_2_frame >> COG_to_REF.GetInverse())))    # i
...
while application.GetDevice().run():
    ...
    application.DrawAll()
    chronoirr.ChIrrTools_drawAllCOGs(system,application.GetVideoDriver(),.5)
    application.DoStep()
