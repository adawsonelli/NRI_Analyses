...
#timing experiments averaging over entire window
import time
tic = time.perf_counter()
mysystem.SetChTime(0)
while (mysystem.GetChTime() < 2):
    mysystem.DoStepDynamics(0.01)
toc = time.perf_counter()

ter = 2 / (toc-tic)
print(" with .01 step, time execution ratio = ", ter) 
...
ter = 2 / (toc-tic)
print(" with .001 step, time execution ratio = ", ter) 




#timing experiment for variability between simulation timepoints
dts = np.zeros(2001)
...
plt.title("walltime to simulate .001 seconds of dynamics of 2DOF robot")
plt.hist(dts,30)
